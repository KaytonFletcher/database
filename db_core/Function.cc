...
Type Function ::RecursivelyBuild(struct FuncOperator *parseTree,
  ...
  if (parseTree->right == 0 && parseTree->leftOperand == 0 &&
    ...
    if (parseTree->leftOperand->code == NAME) {
      ...
      if (myType == String) {
        std::cerr << "Error!  No arithmatic ops over strings are allowed.\n";
        exit(1);
      }
...
void Function ::Print() {
  // std::cout << "Number of Ops for function: " << this->numOps << std::endl;
  // for (int i = 0; i < numOps; i++) {
  //   std::cout << "Operation: " << i << " equals: " << this->opList[i].recInput
  //             << std::endl;
  //   std::cout << "Operation is: " << this->opList[i].myOp << std::endl;
  // }
}
...
Type Function ::Apply(Record &toMe, int &intResult, double &doubleResult) {
  ...
  for (int i = 0; i < numOps; i++) {
    ...
    case PushInt:
      ...
      if (opList[i].recInput >= 0) {
        int pointer = ((int *)toMe.GetBits())[opList[i].recInput + 1];
    ...
    default:
      ...
      std::cerr << "Had a function operation I did not recognize!\n";
      exit(1);
    }
  }
  ...
  if (lastPos != stack) {
    ...
    std::cerr << "left on the stack.  BAD!\n";
    exit(1);
  }
